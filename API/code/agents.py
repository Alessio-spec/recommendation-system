...
import xgboost
from custom_exception import InputError
...
class Activity_Agent:
    ...
    def get_Xtrain(self, df, date, start="2023-11-01", target="activity"):
        import pandas as pd
        ...
        if type(start) == int:
            ...
            start = (
                ...
                if start < pd.to_datetime("2023-11-01")
                else start
            )
        else:
    ...
    def get_ytrain(self, df, date, start="2023-11-01", target="activity"):
        import pandas as pd
    ...
    def train_test_split(
        self, df, date, train_start="2023-11-01", test_delta="all", target="activity"
    ):
    ...
    def evaluate(
            self, df, model_type, split_params, predict_start="2024-01-01", predict_end=-1, return_errors=False,
...
class Load_Agent:
    ...
    def load_profile_raw(self, df, shiftable_devices):
        ...
        for idx, appliance in enumerate(
            ...
            for i in range(len(column)):
                ...
                if (i == 0) and (column.iloc[0] > 0):
                    df_hours[appliance].loc[0, "h" + str(1)] = column[0]

                elif (column.iloc[i - 1] == 0) and (column.iloc[i] > 0):
                    for j in range(0, 24):
                        if (i + j) < len(column):
                            if column.iloc[i + j] > 0:
                                df_hours[appliance].loc[i, "h" + str(j + 1)] = column.iloc[
                                    i + j
                                ]
        return df_hours
    ...
    def load_profile(self, df_hours, shiftable_devices):
        ...
        loads = loads.fillna(0).infer_objects()
...
class Usage_Agent:
    ...
    # train test split
    # -------------------------------------------------------------------------------------------
    def train_test_split(self, df, date, train_start="2023-11-01"):
    ...
    def evaluate(
            self, df, model_type, train_start, predict_start="2024-01-01", predict_end=-1, return_errors=False,
            weather_sel=False, xai=False, **args
    ):
        import pandas as pd
        import numpy as np
...
class X_Recommendation_Agent:
    ...
    def pipeline(self, date, activity_prob_threshold, usage_prob_threshold, evaluation=False, weather_sel=False):
        ...
        for device in self.shiftable_devices[1:]:
            ...
            recommendations_table = recommendations_table._append(
                pd.DataFrame.from_dict(recommendations_by_device)
            )
        return recommendations_table
    ...
    def visualize_recommendation(self, recommendations_table, price, diagnostics=False):
        ...
        # Initialize output here to ensure it's always a list
        output = {
            "no_recommendation": [],
            "recommendation": [],
            "explanations": []
        ...
        for r in range(len(recommendations_table)):
            if recommendations_table.no_recommend_flag_activity.iloc[r] == 0 and \
                ...
                break  # Exit the loop if a recommendation is found
        ...
        if recommendations:
            ...
            explanation_activity = self.Explainability_Agent.explanation_from_feature_importance_activity(
            ...
            for i in range(len(recommendations_table)):
                if recommendations_table.no_recommend_flag_activity.iloc[i] == 0 and \
                        recommendations_table.no_recommend_flag_usage.iloc[i] == 0:
                    date_and_time_str = f"{recommendations_table.recommendation_date.iloc[i]}:{recommendations_table.best_launch_hour.iloc[i]}"
                    date_and_time = datetime.strptime(date_and_time_str, '%Y-%m-%d:%H')
                    # date_and_time_augmented = date_and_time + relativedelta(years=10, month=4)
                    date_and_time_show = date_and_time.strftime(format="%d.%m.%Y %H:%M")
                    date_and_time_price = date_and_time.strftime(format="%Y-%m-%d %H:%M:%S")

                    price_rec = price.filter(like=date_and_time_price, axis=0)['Price_at_H+0'].iloc[0]
                    price_mean = price['Price_at_H+0'].mean()
                    price_dif = price_rec / price_mean
                    price_savings_percentage = round((1 - price_dif) * 100, 2)

                    recommendation_message = f'You have a recommendation for the following device: {recommendations_table.device.iloc[i]}\n\nPlease use the device on the {date_and_time_show[:10]} at {date_and_time_show[11:]} o\'clock because it saves you {price_savings_percentage}% of costs compared to the mean of the day.\n'
                    output["recommendation"].append(recommendation_message)

                    feature_importance_usage_device = recommendations_table['feature_importance_usage'].iloc[i]
                    explanation_usage.append(self.Explainability_Agent.explanation_from_feature_importance_usage(
                        feature_importance_usage_device, date=date, diagnostics=self.diagnostics))
                else:
                    # Append a message indicating no recommendation instead of reassigning output
                    no_recommendation_message = f'There is no recommendation for the device {recommendations_table.device.iloc[i]}.'
                    output["no_recommendation"].append(no_recommendation_message)

            # # Print explanations outside the loop
            # for explanation in explanation_usage:
            #     print(explanation)
            #
            # print(explanation_activity)
            output["explanations"] = [explanation_activity] + explanation_usage

        else:
            no_recommendations_message = 'There are no recommendations for today.'
            print(no_recommendations_message)
            # Append the message to output instead of returning None
            output["no_recommendation"].append(no_recommendations_message)
...
class Evaluation_Agent:
    def __init__(self, DATA_PATH, model_type, config, load_data=True, load_files=None, weather_sel=False, xai = False):
        ...
        self.price = (
            agents.Price_Agent(helper.create_day_ahead_prices_df(DATA_PATH, "Day-ahead Prices_202501010000-202601010000.csv"))
            if load_data
            else None
        )
        self.activity = None
